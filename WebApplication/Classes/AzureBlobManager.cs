...
namespace WebApplication.Classes
{
    ...
    *********************************************************************************/
    ...
    public class AzureBlobManager
    {
        ...
        public void ConvertBlobs(string containerName)
        {
            var container = _blobClient.GetContainerReference(containerName);
            var blobsList = container.ListBlobs(useFlatBlobListing: true);

            foreach (var item in blobsList)
            {
                var blob          = (CloudBlockBlob)item;
                var blobAsByteArr = GetBlobAsByteArr(blob);
                ConvertByteArrToImgList(blobAsByteArr);
            }
        }


        /// <summary> Stores Images returned from blob storage. </summary>
        ...
        public void ConvertByteArrToImgList(byte[] imageByteArr)
        {
            if (imageByteArr == null || imageByteArr.Length == 0) return;
        ...
        /// </summary>
        /// <param name="images">List of Images</param>
        /// <param name="bShouldSaveLocalCopy">Should we save this file to a local drive?</param>
        private void CombineImages(List<Image> images, bool bShouldSaveLocalCopy)
        {
            //change the location to store the final image.
            Debug.WriteLine("DEFINE LOCAL LOCATION HERE: ");
            string finalImage = @"C:\";
            List<int> imageHeights = new List<int>();
            int nIndex             = 0;
            int width              = 0;
